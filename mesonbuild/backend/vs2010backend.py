...
class Vs2010Backend(backends.Backend):
    ...
    def generate_custom_generator_commands(self, target, parent_node):
        ...
        for genlist in target.get_generated_sources():
            if isinstance(genlist, build.GeneratedList):
                generator_output_files.extend(self.generate_genlist_commands(
                    target, genlist, parent_node, down))
            else:
    ...
    def generate_genlist_commands(self, genlist: build.GeneratedList, target, parent_node, down):
        # first generate any chained dependencies
        for dep in genlist.depends:
            if isinstance(dep, build.GeneratedList):
                self.generate_genlist_commands(dep, target, parent_node, down)
        ...
        for i, curfile in enumerate(infilelist):
            if len(infilelist) == len(outfilelist):
                ...
                sole_output = ''
            if isinstance(curfile, build.GeneratedFile):
                file = curfile.to_file(self.get_target_private_dir(target))
            else:
                file = curfile
            infilename = os.path.join(down, file.rel_to_builddir(self.build_to_src))
